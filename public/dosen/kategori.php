...
if (isset($_POST["submit"])) {
  if ($hasil = tambahCategory() > 0) {
    $_SESSION["success"] = "Berhasil Tambah Kategori ";
    $successMsg = $_SESSION["success"];
    unset($_SESSION["success"]);
  } else {
    $_SESSION["error"] = "Gagal Tambah Kategori";
    $errorMsg = $_SESSION["error"];
    unset($_SESSION["error"]);
  }
}
if (isset($_POST["delete"])) {
  if (($hasil = hapusCategory()) > 0) {
    $_SESSION["success"] = $hasil;
    $successMsg = $_SESSION["success"];
    unset($_SESSION["success"]);
  } else {
    $_SESSION["error"] = $hasil;
    $errorMsg = $_SESSION["error"];
    unset($_SESSION["error"]);
  }
}
...
<body>
    ...
    <div id="content">
      ...
      <div class="bg-white p-2 mt-4">
        <button type="button" class="btn btn-success my-2 " data-bs-toggle="modal"
          data-bs-target="#modalKateogri">
          Tambah kateogri Bimbingan <i class="ml-1 fas fa-plus"></i>
        </button>
  ...
  <div class="modal" id="modalKateogri" tabindex="-1">
  ...
  <script>
    ...
    $(document).ready(function() {
      var t = $("#example").DataTable({
        ajax: "../../controller/datatableCategoryControllerDosen.php",
        processing: true,
        serverSide: true,
        "columnDefs": [{
          "searchable": false,
